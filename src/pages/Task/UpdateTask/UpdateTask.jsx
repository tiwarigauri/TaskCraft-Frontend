...
const UpdateTaskForm = ({ open, handleClose, taskId }) => {
  ...
  const formatDate = (input) => {
    if (!input || !(input instanceof Date) || isNaN(input)) {
      console.error("Invalid input for formatDate:", input);
    ...
    if (isNaN(date)) {
      console.error("Formatted date is invalid:", date);
      return null; // Handle the invalid date scenario
    }
  ...
  const handleSubmit = (e) => {
    ...
    console.log("Deadline before formatting:", deadline);
  ...
  useEffect(() => {
    if (task.taskDetails) {
      setFormData({
        ...task.taskDetails,
        deadline: task.taskDetails.deadline ? new Date(task.taskDetails.deadline) : new Date(),
      });
    }
  }, [task.taskDetails]);
